...
# =============================================================================
# Create Model and Add Sensitivity Parameters
# =============================================================================
# 먼저 모델 객체를 생성합니다.
model = Model(sys, [], exception_hook='raise')

# 민감도 분석 변수 추가 (모델의 parameter 컨테이너 사용)
model.parameter.add(
    name='ED Membrane Area',
    element=F.S401,
    kind='coupled',
    units='m^2',
    baseline=F.S401.A_m,
    distribution=shape.Triangle(0.8 * F.S401.A_m, F.S401.A_m, 1.2 * F.S401.A_m),
    setter=lambda x: setattr(F.S401, 'A_m', x)
)

model.parameter.add(
    name='ED Current Density',
    element=F.S401,
    kind='coupled',
    units='A/m^2',
    baseline=F.S401.j,
    distribution=shape.Triangle(0.8 * F.S401.j, F.S401.j, 1.2 * F.S401.j),
    setter=lambda x: setattr(F.S401, 'j', x)
)

model.parameter.add(
    name='AC Tank Residence Time',
    element=F.ac_tank,
    kind='coupled',
    units='hr',
    baseline=F.ac_tank.tau,
    distribution=shape.Triangle(0.8 * F.ac_tank.tau, F.ac_tank.tau, 1.2 * F.ac_tank.tau),
    setter=lambda x: setattr(F.ac_tank, 'tau', x)
)
